...
if (abilReady(e, "mark")) {
abilFire(e, "mark");
ents.forEach(ae => { if (ae === e) return; const acb = C.combat.get(ae), atm = C.team.get(ae); if (atm.team === tm.team && !acb.dead) { const ap = C.pos.get(ae); if (hypot(ap.x - p.x, ap.y - p.y) < markRadius(b)) { acb.markTarget = target; acb.markT = ABILITIES.mark.dur } } });
tcb.markFlash = 1200;
}
